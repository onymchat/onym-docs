...
*Role-binding page, draft 0.1 — 15 August 2026.*
...
| Financial provider | None bound. Fiat rails under the operator's own authority are the assumed starting point on both chain pages, but *assumed* is not *bound*: no `financialBindings` entry exists because no deployment exists. | — | **Unassigned**, and partly **out of scope by design** — both chain profiles bind notary and eligibility roles only. |
